...
class Product extends BaseMapper
{
    ...
    protected function addVarCombiAsVariation($data, $masterId)
    {
        ...
        if (!isset($variationId)) {
            $id = (int)$this->db->query("SELECT products_options_id FROM products_options ORDER BY products_options_id DESC LIMIT 0,1");
        ...
        $variationOptionId = $this->db->query("SELECT products_options_values_id FROM products_options_values ORDER BY products_options_values_id DESC LIMIT 0,1");
        if (isset($variationOptionId[0]["products_options_values_id"])){
            $variationOptionId = ((int)$variationOptionId[0]["products_options_values_id"]) +1;
        }else{
            $variationOptionId = 0;
        }
        $variationOptionName = $data->getVariations()[0]->getValues()[0]->getI18ns()[0]->getName();
        ...
        $price = $data->getPrices()[0]->getItems()[0]->getNetPrice();
        $pricePrefix = "+";
        $model = "null";
        ...
        $this->db->query('
            ...
            WHERE products_options_id IN (
                SELECT * FROM (
                    ...
                    LEFT JOIN jtl_connector_link_product l ON o.products_options_id = l.products_options_id
                    WHERE l.products_options_id IS NULL
